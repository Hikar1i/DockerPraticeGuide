...
![img.png](_images/dockerstatus.png)
...
| **`ARG`**        | 用于在构建镜像的过程中定义变量。这些变量只在 docker build 的过程中有效。构建好的镜像内不存在这些变量。可以通过 docker build --build-arg <argname>=<value> 命令在构建时为其赋值。 | `ARG GIT_USER=chenwg`                    |
| **`EXPOSE`**     | **声明**容器运行时监听的端口（只是一个说明, 实际映射需用 `-p`）。                                                                                | `EXPOSE 80`                              |
| **`VOLUME`**     | 将容器的指定路径进行持久化，会产生随机（匿名）存储卷。                                                                                | `VOLUME ["/data", "/logs", "/config"]`                              |
...
| 情况                | 命令示例                             | 结果      | 数据持久性     |
| ----------------- | -------------------------------- | ------- | --------- |
| **无 VOLUME，无 -v** | `docker run app`                 | 数据在容器层  | ❌ 容器删除即丢失 |
| **有 VOLUME，无 -v** | `docker run app`                 | 自动创建匿名卷 | ✅ 数据持久化   |
| **有 VOLUME，有 -v** | `docker run -v my-vol:/data app` | 使用指定卷   | ✅ 数据持久化   |
...
- 应用需要 **编译** 或 **打包** (例如 Java、Go、前端项目), 编译环境依赖庞大, 但运行时只需要少量依赖。
- 如果把编译环境和运行环境放到同一个镜像里, 镜像体积会很大, 而且包含了很多运行时不需要的工具。

**多阶段构建** 就是为了解决这个问题的, 它允许在一个 Dockerfile 中使用多个 `FROM` 阶段, 并且可以从前一个阶段复制需要的产物到后一个阶段,
最终只保留运行所需的内容。
...
```shell
docker run -d --name test-springboot-container -p 8484:8484 test-springboot:v1
```

### 4. 验证启动

现在, 可以通过访问 `http://localhost:8484/hello` 来访问容器中的应用页面。

![img.png](_images/dockerfile-app.png)
...
3. Vue 前端服务, 命名为vue-fe-app
    ...
    - environment: 设置时区为上海。
    - ports: 将宿主机 8585 端口映射到容器 80端口, 用于对外提供前端访问。
...
访问 http://localhost:8585 即可看到前端项目页面
...
## 9.3. docker-compose 多容器互联和网络模式

在 `docker-compose` 中, 多个服务通常需要互相访问（例如: Web 应用访问数据库）。

以下 **`test-webapp` (Web 应用) + `test-database` (数据库)** 的组合演示三种模式的区别。

### 1.默认Bridge网络 (默认模式)

- 当使用 `docker-compose up` 启动时, **compose 默认会创建一个独立的 bridge 网络**。
- 容器之间可以通过 **服务名直接访问**，无需关心实际 IP。
...
services:
  test-webapp:
    ...
    environment:
      JDBC_URL: jdbc:mysql://test-database:3306/mydb
    depends_on:
      - test-database
...
* 在 `test-webapp` 内, 数据库地址直接写成 `jdbc:mysql://test-database:3306/mydb`。
* **服务名 `test-database` 即可作为数据库主机名**。

### 2. 自定义网络 (bridge)

在实际项目中，有时需要对容器进行分组管理，让同一组内的容器可以互相访问，而组与组之间保持网络隔离。  
这种情况下，可以创建多个自定义 `bridge` 网络，每组容器加入到各自的网络里。

**示例: 两个 WebApp + 两个 Database，分别放入两个不同的网络：**
...
services:
  ...
  test-webapp-1:
    image: my-webapp:latest
    environment:
      JDBC_URL: jdbc:mysql://test-database-1:3306/mydb # 访问不通test-database-2
    depends_on:
      - test-database-1
    networks:
      - app_net_group1
  ...
  test-webapp-2:
    image: my-webapp:latest
    environment:
      JDBC_URL: jdbc:mysql://test-database-2:3306/mydb # 访问不通test-database-1
    depends_on:
      - test-database-2
    networks:
      - app_net_group2
...
* 在 `test-webapp-1` 内，数据库地址为 `jdbc:mysql://test-database-1:3306/mydb`，只能访问 `test-database-1`。
* 在 `test-webapp-2` 内，数据库地址为 `jdbc:mysql://test-database-2:3306/mydb`，只能访问 `test-database-2`。
* **组内互通，组间隔离**:

    * `test-webapp-1` **不能访问** `test-database-2`。
    * `test-webapp-2` **不能访问** `test-database-1`。

* 使用多个自定义 `bridge` 网络，可以将不同业务分组隔离，既保证组内容器互通，又避免不相关服务之间的干扰。

### 3. Host 模式

* 使用 `network_mode: host` 后, 容器和宿主机共享网络栈, 没有独立的容器网络。
* 此时容器之间不能用服务名访问, 只能通过 **宿主机 IP** 或 `localhost`。
...
* 在 `test-webapp` 内, 访问数据库必须写 `jdbc:mysql://127.0.0.1:3306/mydb` (即宿主机地址)。
* 不能再使用服务名 `test-database` 作为主机名。

### 4. 总结对比

| 网络模式           | JDBC\_URL 示例                           | 访问方式                 |
|----------------|----------------------------------------|----------------------|
| **默认 bridge**  | `jdbc:mysql://test-database:3306/mydb` | 通过服务名互通 (推荐)         |
| **自定义 bridge** | `jdbc:mysql://test-database:3306/mydb` | 服务名互通, 可跨 compose 项目 |
| **host**       | `jdbc:mysql://127.0.0.1:3306/mydb`     | 直接走宿主机网络, 不能用服务名     |
...
Linux环境中的docker配置host模式, 容器内可以直接使用 `localhost` 访问宿主机。

但由于Windows的Docker Desktop, 容器实际是运行在一个轻量级 Linux VM 中, VM中 `localhost` 指向的是 VM 自己, 而不是windows宿主机

从 Windows 宿主机访问容器映射的端口, 通常使用 `localhost:端口` 是可行的, 但容器内访问宿主机时, 不能使用 `localhost`。
